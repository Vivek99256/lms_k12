...
/**
 * Attendance → AI Stack → Policies.
 *
 * What the AI is allowed to do in Attendance, who may operate it, and what it must
 * disclose when it does.
 *
 * DECENTRALISED, NOT DUPLICATED
 *
 * Every row here is an `ai_policies` row reached through the same
 * `lib/intelligence/ai-policies` client the central console and the Fees screen use.
 * There is no second policy store and no second set of endpoints. What makes it
 * Attendance is the scope: the list is fetched with `module_key=attendance`, so it only
 * ever contains policies carrying a `module` assignment for Attendance, and every save
 ...
 * `scope_type = 'module'` with `scope_id` set to the Attendance row in `ai_modules`.
 * Nothing was added to the database to hold it. The ids come from `/policies/options`,
 * never from a constant here: they differ per estate, and a hardcoded number would
 * silently scope a policy to whatever module happened to be at that id.
 *
 * WHY "WHO MAY OPERATE IT" IS SHOWN HERE AND NOT EDITED HERE
 *
 * A policy says what the AI may do. It does not say who may do it — that is RBAC, it
 * lives in `tblgroupwise_rights` against a menu row, and it is granted in Group-wise
 * Rights like every other right in this ERP. Building a second place to grant it would
 * be a second authorization system, and the one thing worse than a permission nobody
 * holds is two answers to who holds it.
 *
 * So the panel below reads the caller's own resolved rights and names the key, because
 * the failure this replaces was a screen that said "ask an administrator for
 * agents.attendance rights" without anywhere to see whether you had them or what the key
 * even was. It shows; it does not grant.
 */
...
function blankForm(options: AiPolicyOptions | null): FormState {
  return {
    ...
    status: 1,
    // On by default, and it matters more here than almost anywhere else: a family reading
    // a message about their child's absence should be told a machine drafted it.
...
function formFrom(row: AiPolicyRow): FormState {
  return {
    ...
    // manage has to be handed back or editing an Attendance policy here would quietly
...
export function AttendancePoliciesScreen() {
  const [options, setOptions] = useState<AiPolicyOptions | null>(null);
  const [rows, setRows] = useState<AiPolicyRow[]>([]);
  /** The `ai_modules` ids Attendance resolves to. A save needs one; without any, it cannot. */
  ...
  useEffect(() => {
    ...
    Promise.all([fetchAiPolicyOptions(), fetchAiPolicies(ATTENDANCE_MODULE)])
      .then(([nextOptions, index]) => {
        ...
        // every module. Either answers "which id is Attendance", and neither is hardcoded.
        setModuleIds(
          index.module_ids?.length
            ? index.module_ids
            : (nextOptions.modules ?? [])
                .filter((module) => module.key === ATTENDANCE_MODULE)
  ...
  /**
   * The Attendance scope every save writes.
   *
   ...
   */
  const attendanceScopeId = moduleIds.length ? Math.max(...moduleIds) : null;

  const canSave = attendanceScopeId !== null;

  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!form || attendanceScopeId === null) return;
    ...
    const payload = {
      ...
      rules: form.rules,
      // The Attendance assignment is added by this screen, not chosen by the operator —
      // it is what makes the policy an Attendance policy at all.
      assignments: [
        { scope_type: 'module', scope_id: attendanceScopeId, status: 1 },
    ...
    try {
      if (form.id === null) {
        ...
        setNotice('Attendance AI policy saved. It applies to the Attendance module only.');
      } else {
        await updateAiPolicy(form.id, payload);
        setNotice('Attendance AI policy updated.');
      }
  ...
  const retire = async (row: AiPolicyRow) => {
    if (!window.confirm(`Retire "${row.name}"? Attendance AI will stop applying it.`)) return;
  ...
  if (loading && !options) {
    return <AiStackLoading label="Loading Attendance AI policies…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        ...
        title="Attendance AI policies"
        summary="What the AI may be used for inside Attendance, who may operate it, and what it has to disclose. These policies apply to the Attendance module only."
        loading={loading}
        onRefresh={reload}
        actions={
          <button
            ...
            disabled={!canSave}
            title={canSave ? undefined : 'Attendance is not registered in ai_modules on this estate.'}
      ...
      <AiStackHint>
        A policy saved here is scoped to Attendance — it is stored against the Attendance module and no other module
      ...
      {!canSave && !loading && (
        <AiStackError>
          Attendance has no row in <span className="font-mono">ai_modules</span> for this institute, so a policy cannot
          be scoped to it. An administrator needs to register the Attendance module before policies can be saved here.
        </AiStackError>
      )}
      ...
      {rows.length === 0 && !loading ? (
        <AiStackEmpty
          icon={ShieldCheck}
          title="No Attendance AI policies yet"
          action={
            canSave ? (
              <button
                type="button"
        ...
        >
          Until one exists, Attendance AI runs under whatever the estate-wide policies allow. A policy created here
          narrows that to Attendance.
        </AiStackEmpty>
      ) : (
        ...
        <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-base font-semibold text-slate-950">
              {form.id === null ? 'New Attendance AI policy' : 'Edit Attendance AI policy'}
            </h3>
            <button
              type="button"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              />
              <span className="mt-1 block text-xs text-slate-500">This policy applies to Attendance only.</span>
            </label>
            ...
            <label className="block">
              ...
              <input
                ...
                required
                maxLength={191}
                placeholder="Attendance follow-up drafting policy"
          ...
          <div className="block">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="attendance-policy-description" className="text-sm font-medium text-slate-900">
              ...
              <AiFieldAssistant
                ...
                entityType="attendance_ai_policy"
                maxLength={2000}
          ...
          <fieldset className="rounded-xl border border-slate-200 p-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Toggle
                ...
                hint="An AI-assisted attendance message says so."
          ...
          <fieldset className="rounded-xl border border-slate-200 p-4">
            <legend className="px-1 text-sm font-semibold text-slate-900">Allowed AI uses in Attendance</legend>
...
/**
 * Your own rights over Attendance AI, and the key they are granted against.
 *
 * Read-only by design — see the note at the top of this file. It answers the two
 * questions a person actually has when a control is disabled: what am I allowed to do,
 * and what is the thing an administrator has to grant me.
 *
...
function OperatorRightsPanel() {
  const moduleKey = rbacModuleKey(ATTENDANCE_MODULE);
  ...
  const cells: Array<{ action: 'view' | 'create' | 'update' | 'delete'; label: string; hint: string }> = [
    { action: 'view', label: 'See Attendance agents', hint: 'Open Automations and read the run log.' },
    { action: 'create', label: 'Enable an agent', hint: 'Switch on an Attendance agent for this school.' },
    { action: 'update', label: 'Run or pause an agent', hint: 'Run an agent, and pause or resume one.' },
    { action: 'delete', label: 'Remove an agent', hint: 'Archive an Attendance agent.' },
  ];

  return (
    <AiStackCard>
      <AiStackCardHeading
        title="Who may operate Attendance AI"
