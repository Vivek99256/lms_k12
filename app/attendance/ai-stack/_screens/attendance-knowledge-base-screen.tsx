...
/**
 * Attendance → AI Stack → Knowledge Base.
 *
 * The attendance material the AI is allowed to draw on when it answers, and proof that
 * each source actually answers.
 *
 * WHAT A "KNOWLEDGE SOURCE" IS HERE
 *
 * Not a folder of uploaded files. For Attendance it is a read-only MCP tool —
 * `attendance.overview`, `attendance.student` — because that is what the assistant really
 * reads when somebody asks an attendance question. Those tools already carry the tenant
 * scoping, the academic-year filter, the attendance-code rules and the "too few marked
 * days to judge" threshold that the rest of the platform was built on, and
 * `ReportDataSourceCatalog` in Laravel lists them from the tool registry itself. So the
 * catalogue on this screen is derived, not maintained: register an attendance tool in the
 ...
 * altered a child's register.
 *
 * WHY THERE IS A CHECK BUTTON
 *
 * A source can be registered, bound to a template, and still return nothing for this
 * school — wrong academic year, no marked days, a permission the signed-in user lacks.
 * That is invisible on an inventory and obvious the moment you call it. Check runs the
 * real tool through the existing `/api/mcp/tools/call` proxy, as the signed-in user, and
 * reports what came back. It reads; it writes nothing.
 *
 * NOTHING IS SEEDED INTO THIS TAB. There is no sample document and no fabricated
 * attendance policy. An estate that has indexed nothing is told exactly that, because a
 * knowledge base padded with invented material is worse than an empty one — it looks
 * authoritative and it is not.
 */
...
import { ATTENDANCE_MODULE, readAttendanceWorkspaceSession } from '@/lib/attendance/attendance-ai-stack';
...
export function AttendanceKnowledgeBaseScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchTemplateOptions(), fetchTemplates(ATTENDANCE_MODULE)])
  ...
  /** Attendance sources only. This screen never lists another module's tools. */
  const sources = useMemo<TemplateDataSource[]>(
    () => (options?.data_sources ?? []).filter((source) => source.module === ATTENDANCE_MODULE),
    [options],
  );

  /** Which Attendance templates read each source — the binding, from the template rows. */
  ...
  const check = useCallback(async (source: TemplateDataSource) => {
    ...
    const session = readAttendanceWorkspaceSession();
    ...
    try {
      const response = await fetch('/api/mcp/tools/call', {
        ...
        body: JSON.stringify({
          tool: source.name,
          // No arguments, so a tool with a required argument reports itself as such
          // rather than being called wrongly. `attendance.student` needs a student and
  ...
  if (loading && !options) {
    return <AiStackLoading label="Loading Attendance knowledge sources…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={BookMarked}
        title="Attendance knowledge base"
        summary="The attendance records and documents the AI may draw on when it answers, and whether each one is actually returning data."
        loading={loading}
        onRefresh={reload}
      />
      ...
      <AiStackMetrics
        metrics={[
          { key: 'sources', label: 'Attendance sources', value: sources.length, hint: 'read-only tools' },
          { key: 'bound', label: 'In use', value: boundCount, hint: 'read by an Attendance template' },
          { key: 'templates', label: 'Attendance templates', value: templates.length, hint: 'prompts and reports' },
          {
            key: 'documents',
            label: 'SOP documents',
            value: documentCount ?? '—',
      ...
      <AiStackHint>
        ...
        annotation, so a tool that marks a register cannot appear here or be bound to a template.
      </AiStackHint>

      {sources.length === 0 ? (
        <AiStackEmpty icon={Database} title="No Attendance knowledge sources registered">
          The backend reported no read-only attendance tools. Until one is registered, Attendance AI has no register to
          ground an answer in.
        </AiStackEmpty>
      ) : (
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Attendance data sources"
      ...
      <AiStackCard className="overflow-hidden">
        <AiStackCardHeading
          ...
          hint="SOPs and circulars from ai_sops. Held per institute, not per module — an attendance policy indexed here is visible to the whole estate's AI."
        />
        ...
        {documentsError ? (
          ...
          <p className="px-5 py-6 text-sm text-slate-500">
            No documents are indexed for this institute yet. Attendance answers therefore rest entirely on the marked
            register above, which is the stronger ground of the two.
          </p>
        )}
      </AiStackCard>
    </section>
...
function summariseMcpPayload(payload: Record<string, unknown> | null): { rows: number | null; detail: string } {
  ...
  const candidates: unknown[] = [
    ...
    // `attendance.overview` returns its list under `students`. Named explicitly rather
    // than left to the generic keys below, so the count is the cohort and not whichever
    // array happened to be found first.
