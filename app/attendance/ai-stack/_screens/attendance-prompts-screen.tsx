...
/**
 * Attendance → AI Stack → Prompts.
 *
 * The prompts behind each Attendance AI feature, kept out of the code.
 *
 * WHY THIS SITS BESIDE TEMPLATES RATHER THAN INSIDE IT
 *
 * A prompt and a report template are both `ai_templates` rows for
 * `module_key = 'attendance'`, separated by `kind`. They are not the same thing and must
 * not share a list: a prompt is text sent to a model, which writes prose; a report is an
 * HTML layout whose placeholders are filled by substitution from rows a tool fetched,
 * with no model near the figures. That is why a report can print "70.7%" and a prompt can
 * only describe it. The Templates screen filters to `kind === 'report'` and this one to
 * `kind === 'prompt'`, so an author editing "the low attendance prompt" cannot land in
 ...
 * Fees screen call, so the contract is shared and a change to it breaks all three at
 * compile time. What is Attendance about this one is the scope: the list is fetched with
 * `module_key=attendance` and every save sets it, so a prompt written here cannot come
 * out filed against another module.
 *
 * PLATFORM PROMPTS ARE NOT EDITED IN PLACE
 *
 * The five Attendance prompts that ship with the product are platform rows — shared by
 * every school, owned by none. The API reports that as `editable_in_place: false`, and
 * saving one writes this school its own copy rather than changing everybody's. The screen
 * says so before the save rather than after, because "I edited the shared prompt" and "I
...
import { ATTENDANCE_MODULE, refreshAttendanceAiStack } from '@/lib/attendance/attendance-ai-stack';
...
function blankForm(options: AiTemplateOptions | null): FormState {
  return {
    ...
    name: '',
    description: '',
    // The two rules every attendance prompt needs and which are easiest to forget: work
    // only from the register, and never guess why a child was away.
    system_prompt:
      'You answer questions about school attendance. Use only the attendance records you are given. If a figure is not in them, say so rather than estimating. The register records that a student was absent, never why — so never state or imply a reason for an absence.',
    user_prompt: '',
    status: 'draft',
    category: '',
...
export function AttendancePromptsScreen() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (form.id === null) {
        ...
        setNotice('Attendance prompt saved.');
      } else {
        ...
        setNotice(
          result.action === 'forked' || form.forks
            ...
            : 'Attendance prompt updated.',
        );
      }

      setForm(null);
      setPreview(null);
      // The screens that resolve a prompt for an operation cache the published set for
      // the lifetime of the page. Dropping it here means the prompt just published is the
      // one the next draft uses, rather than the one that was published when the tab
      // opened.
  ...
  const retire = async (row: AiTemplateRow) => {
    if (!window.confirm(`Retire "${row.name}"? Attendance AI will stop using it.`)) return;
  ...
  if (loading && rows.length === 0 && !error) {
    return <AiStackLoading label="Loading Attendance prompts…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        ...
        summary="The text sent to a model behind each Attendance AI feature. Versioned, editable, and never compiled into the app."
        loading={loading}
        onRefresh={reload}
        actions={
          <button
      ...
      <AiStackHint>
        A prompt is text a model reads; it cannot state an attendance figure on its own authority. Anything that has to
        print real percentages belongs on the <strong>Templates</strong> tab, where the figures come from the marked
        register rather than a model.
      </AiStackHint>
      ...
      {rows.length === 0 && !loading ? (
        <AiStackEmpty
          icon={Terminal}
          title="No Attendance prompts yet"
          action={
            <button
              type="button"
        ...
        >
          Attendance AI features fall back to their built-in wording until a prompt exists here.
        </AiStackEmpty>
      ) : (
        <AiStackCard className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[60rem] border-collapse text-left text-sm">
              <AiStackTableHead
                columns={['Prompt', 'Grounding', 'Version', 'Review', 'In Attendance', 'Status', 'Updated', 'Actions']}
              />
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-base font-semibold text-slate-950">
              {form.id === null
                ? 'New Attendance prompt'
                : form.forks
                  ? 'Customise for this institute'
                  : 'Edit Attendance prompt'}
            </h3>
            <button
              type="button"
              onClick={() => {
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              {/* Fixed, and shown rather than hidden: this screen exists to manage
                  Attendance prompts, and a module selector here would offer one option. */}
              <input
                value="Attendance"
                readOnly
          ...
          <div className="block">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="attendance-prompt-description" className="text-sm font-medium text-slate-900">
                What it is for
              </label>
              <AiFieldAssistant
                ...
                entityType="attendance_ai_prompt"
                related={{ Prompt: form.name }}
              />
            </div>
            <input
              id="attendance-prompt-description"
              value={form.description}
          ...
          {variables.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Click one to insert it. Green variables carry the attendance data a grounded answer has to rest on — a
                prompt with none of them is refused before a model is called.
              </p>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <label className="block">
              ...
              <input
                ...
                list="attendance-prompt-categories"
                className="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
              />
              <datalist id="attendance-prompt-categories">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <label className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900">
              ...
              <span>
                ...
                <span className="mt-0.5 block text-xs text-slate-500">
                  Nothing generated by this prompt is used until somebody reads it. Leave this on for anything a family
                  will receive.
                </span>
              </span>
            </label>
            ...
            <label className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900">
              ...
              />
              <span>
                Offer this prompt in the Attendance AI panel
                <span className="mt-0.5 block text-xs text-slate-500">
                  It appears as a suggestion when somebody opens the assistant from an Attendance page.
                </span>
              </span>
            </label>
          </div>
 ...
 *
 * No model is called and nothing is stored — this answers "did my placeholder land where
 * I meant it to", which is a question about the text.
 */
