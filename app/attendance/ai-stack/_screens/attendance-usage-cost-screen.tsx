...
/**
 * Attendance → AI Stack → Usage & Cost.
 *
 * What the Attendance module's AI has actually done, and what it cost.
 *
 * ONE CENTRAL USAGE SYSTEM, ASKED A MODULE-SHAPED QUESTION
 *
 * Nothing here is an Attendance-specific meter. The counts come from
 * `/api/ai/modules/attendance/usage` — the same endpoint every module's tab calls, over
 * columns the schema already had: `ai_conversations.module_key`, generations through
 * Attendance templates, reports filed against Attendance, and the quota on whichever
 * credential Attendance resolves to. No table, column or counter was added to make this
 * tab work.
 *
 ...
 * to explain a bill. So when it cannot multiply, this screen shows no money and prints
 * the backend's own reason for it. A plausible number here would be the single most
 * damaging thing it could display.
 *
 * AGENT RUNS ARE COUNTED SEPARATELY, AND SAID TO BE
 *
 * Attendance tool-agent runs live in the Agent Management engine, not in Laravel's
 * conversation tables, so they are read from `/api/agents/runs` and shown in their own
 * panel rather than folded into a single total. Two stores, two counts, both labelled —
 * better than one number that is quietly the sum of different things.
 */
...
export function AttendanceUsageCostScreen() {
  ...
  useEffect(() => {
    ...
    fetchModuleUsage(ATTENDANCE_MODULE)
      .then((next) => {
        if (cancelled) return;
        setUsage(next);
        setError('');
    ...
    fetchRuns({ module: ATTENDANCE_MODULE, limit: 200 })
      .then((next) => {
  ...
  if (loading && !usage) {
    return <AiStackLoading label="Loading Attendance AI usage…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={Gauge}
        title="Attendance AI usage and cost"
        summary="What the Attendance module consumed, who used it, and what it cost. Every figure below is counted from records — nothing is estimated."
        loading={loading}
        onRefresh={reload}
      />
      ...
      {usage && !usage.module.registered && (
        <AiStackError>
          Attendance is not registered as an AI module on this estate, so no usage can be attributed to it.
        </AiStackError>
      )}
      ...
      <AiStackMetrics
        metrics={[
          ...
          },
          {
            ...
            hint: 'asked an Attendance question',
          },
          {
            key: 'latency',
          ...
          },
          {
            ...
            hint: runsError ? 'store unreachable' : 'Attendance tool agents',
          },
          {
            key: 'cost',
            label: 'Cost',
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Attendance conversations"
            hint="Questions asked from an Attendance page, from ai_conversations.module_key."
          />
        ...
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="What Attendance is asking for"
          ...
          {!conversations?.available || !(conversations.turn_detail.by_intent ?? []).length ? (
            <p className="px-5 py-6 text-sm text-slate-500">No questions have been classified for Attendance yet.</p>
          ) : (
                ...
                return (
                  <li key={row.intent} className="px-5 py-2.5">
                    <div className="flex items-center justify-between gap-3 text-sm">
                      <span
                        className={unclassified ? 'font-mono text-xs text-amber-700' : 'font-mono text-xs text-slate-700'}
                      >
                        {row.intent}
                      </span>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            ...
            hint="From generations through Attendance templates, priced at the resolved model's own published rate."
          />
          {!tokens ? (
        ...
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Quota"
            hint="The daily call ceiling on whichever credential Attendance resolves to."
          />
          {!provider?.available ? (
            ...
            <>
              <dl className="divide-y divide-slate-100">
                ...
                <Row
                  label="Binding"
                  value={provider.bound ? 'Attendance has its own credential' : 'shared with every module'}
                />
                <Row
                  label="Daily limit"
              ...
              {!provider.bound && (
                <p className="border-t border-slate-100 bg-amber-50 px-5 py-3 text-xs leading-5 text-amber-900">
                  Attendance has no credential of its own, so its calls, quota and spend cannot be separated from every
                  other module&apos;s. That is the normal state and usually the right one — see the{' '}
      ...
      <AiStackCard className="overflow-hidden">
        <AiStackCardHeading
          title="Attendance tool-agent runs"
          hint="Counted from the Agent Management engine, which is a separate store from the conversation tables above. Runs of the backend Attendance Agent are listed on the Automations tab."
        />
        {runsError ? (
          ...
          <p className="px-5 py-6 text-sm text-slate-500">
            No Attendance tool agent has been run yet. Enable one on the <strong>Automations</strong> tab.
          </p>
        ) : (
          <>
      ...
      {usage && usage.recent_turns.length > 0 && (
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Recent Attendance questions"
      ...
      <p className="flex items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-600">
        ...
        <span>
          Conversations, generations and reports are attributed to Attendance by their own module column. Estate-wide
          AI logs have no module dimension and are deliberately excluded rather than apportioned by guess, so these
          totals are a floor on Attendance usage, not a share of the whole.
        </span>
      </p>
    </section>
  );
