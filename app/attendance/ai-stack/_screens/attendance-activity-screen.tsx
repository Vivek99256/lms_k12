...
/**
 * Attendance → AI Stack → Activity.
 *
 * The ledger. What happened in Attendance, which AI capability it used, which agent,
 * prompt or template ran, who did it, which student it was about, and how it ended.
 *
 * WHY THIS TAB EXISTS
 *
 * The other eight tabs each answer "what is configured". None of them answers "what
 * actually ran", and a stack you cannot audit is a stack nobody should point at a child's
 * record. This is the join: a row here names the Attendance operation on one side and the
 * AI Stack record on the other, so a question like "which prompt drafted the note we sent
 * that family" has an answer that is a lookup rather than an investigation.
 *
 * WHERE THE ROWS COME FROM
 *
 * `ai_audit_logs`, under an event type of `module.attendance.<operation>` — the same table
 * the agent runs, generation requests and governance refusals already write to. No table
 * was added for this. Each row was written by an Attendance screen at the moment it
 * finished doing something, and the template and prompt names in it were resolved by the
 * backend out of `ai_templates` rather than supplied by the browser, so a row cannot name
 * an artefact that does not exist.
 *
 ...
 * the things that worked is a ledger that cannot answer the question people actually
 * bring to it.
 *
 * AN ENTRY WITH NO AI IS STILL AN ENTRY
 *
 * Marking a register uses no model. It is recorded anyway, with no artefact against it,
 * because the ledger's job is to show what happened in Attendance — and "this operation
...
export function AttendanceActivityScreen() {
  ...
  useEffect(() => {
    ...
    fetchModuleActivity(ATTENDANCE_MODULE, { limit: PAGE, operation: operation || undefined })
      .then((next) => {
        if (cancelled) return;
        setActivity(next);
        setError('');
  ...
  if (loading && !activity) {
    return <AiStackLoading label="Loading the Attendance AI activity ledger…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={History}
        title="Attendance AI activity"
        summary="Every Attendance operation the AI Stack recorded — what ran, which capability and record it used, who did it, and how it ended."
        loading={loading}
        onRefresh={reload}
        actions={
          operations.length > 0 ? (
            ...
            >
              ...
              {operations.map(([key, count]) => (
                <option key={key} value={key}>
                  {ATTENDANCE_OPERATIONS[key as AttendanceOperationKey]?.label ?? key} ({count})
                </option>
              ))}
            </select>
          ) : undefined
      ...
      <AiStackMetrics
        metrics={[
          { key: 'total', label: 'Recorded', value: activity?.total ?? 0, hint: 'Attendance operations' },
      ...
      {entries.length === 0 && !loading ? (
        <AiStackEmpty icon={History} title="Nothing recorded yet">
          The ledger fills as people use Attendance AI. Build an attendance report, run the Attendance Agent, or draft a
          note with AI on an Attendance screen, and the operation appears here with whatever AI Stack record it used.
        </AiStackEmpty>
      ) : (
                    ...
                    <td className="px-4 py-2.5">
                      <div className="text-xs font-medium text-slate-900">
                        {entry.operation_label ??
                          ATTENDANCE_OPERATIONS[entry.operation as AttendanceOperationKey]?.label ??
                          entry.operation}
                      </div>
