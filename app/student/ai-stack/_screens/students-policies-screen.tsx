...
/**
 * Student → AI Stack → Policies.
 *
 * What the AI is allowed to do in Students, who may operate it, and what it must
 ...
 * it Students is the scope: the list is fetched with `module_key=students`, so it only
 * ever contains policies carrying a `module` assignment for Students, and every save
 ...
 * `scope_type = 'module'` with `scope_id` set to the Students row in `ai_modules`.
...
function blankForm(options: AiPolicyOptions | null): FormState {
  return {
    ...
    // about their child's record should be told a machine drafted it.
...
function formFrom(row: AiPolicyRow): FormState {
  return {
    ...
    // manage has to be handed back or editing a Student policy here would quietly drop
...
export function StudentsPoliciesScreen() {
  const [options, setOptions] = useState<AiPolicyOptions | null>(null);
  const [rows, setRows] = useState<AiPolicyRow[]>([]);
  /** The `ai_modules` ids Students resolves to. A save needs one; without any, it cannot. */
  ...
  useEffect(() => {
    ...
    Promise.all([fetchAiPolicyOptions(), fetchAiPolicies(STUDENTS_MODULE)])
      .then(([nextOptions, index]) => {
        ...
        // every module. Either answers "which id is Students", and neither is hardcoded.
        setModuleIds(
          index.module_ids?.length
            ? index.module_ids
            : (nextOptions.modules ?? [])
                .filter((module) => module.key === STUDENTS_MODULE)
  ...
  /**
   * The Students scope every save writes.
   *
   ...
   */
  const studentsScopeId = moduleIds.length ? Math.max(...moduleIds) : null;

  const canSave = studentsScopeId !== null;

  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!form || studentsScopeId === null) return;
    ...
    const payload = {
      ...
      rules: form.rules,
      // The Students assignment is added by this screen, not chosen by the operator —
      // it is what makes the policy a Student policy at all.
      assignments: [
        { scope_type: 'module', scope_id: studentsScopeId, status: 1 },
    ...
    try {
      if (form.id === null) {
        ...
        setNotice('Student AI policy saved. It applies to the Student module only.');
      } else {
        await updateAiPolicy(form.id, payload);
        setNotice('Student AI policy updated.');
      }
  ...
  const retire = async (row: AiPolicyRow) => {
    if (!window.confirm(`Retire "${row.name}"? Student AI will stop applying it.`)) return;
  ...
  if (loading && !options) {
    return <AiStackLoading label="Loading Student AI policies…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        ...
        title="Student AI policies"
        summary="What the AI may be used for inside Students, who may operate it, and what it has to disclose. These policies apply to the Student module only."
        loading={loading}
        onRefresh={reload}
        actions={
          <button
            ...
            disabled={!canSave}
            title={canSave ? undefined : 'Students is not registered in ai_modules on this estate.'}
      ...
      <AiStackHint>
        A policy saved here is scoped to Students — it is stored against the Student module and no other module
      ...
      {!canSave && !loading && (
        <AiStackError>
          Students has no row in <span className="font-mono">ai_modules</span> for this institute, so a policy cannot
          be scoped to it. An administrator needs to register the Student module before policies can be saved here.
        </AiStackError>
      )}
      ...
      {rows.length === 0 && !loading ? (
        <AiStackEmpty
          icon={ShieldCheck}
          title="No Student AI policies yet"
          action={
            canSave ? (
              <button
                type="button"
        ...
        >
          Until one exists, Student AI runs under whatever the estate-wide policies allow. A policy created here
          narrows that to Students.
        </AiStackEmpty>
      ) : (
        ...
        <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-base font-semibold text-slate-950">
              {form.id === null ? 'New Student AI policy' : 'Edit Student AI policy'}
            </h3>
            <button
              type="button"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              />
              <span className="mt-1 block text-xs text-slate-500">This policy applies to Students only.</span>
            </label>
            ...
            <label className="block">
              ...
              <input
                ...
                required
                maxLength={191}
                placeholder="Student record request policy"
          ...
          <div className="block">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="students-policy-description" className="text-sm font-medium text-slate-900">
              ...
              <AiFieldAssistant
                ...
                entityType="students_ai_policy"
                maxLength={2000}
                related={{ Policy: form.name }}
              />
            </div>
            <textarea
              id="students-policy-description"
              value={form.description}
          ...
          <fieldset className="rounded-xl border border-slate-200 p-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Toggle
                ...
                hint="An AI-assisted student message says so."
          ...
          <fieldset className="rounded-xl border border-slate-200 p-4">
            <legend className="px-1 text-sm font-semibold text-slate-900">Allowed AI uses in Students</legend>
...
/**
 * Your own rights over Student AI, and the key they are granted against.
 *
...
function OperatorRightsPanel() {
  const moduleKey = rbacModuleKey(STUDENTS_MODULE);
  ...
  const cells: Array<{ action: 'view' | 'create' | 'update' | 'delete'; label: string; hint: string }> = [
    { action: 'view', label: 'See Student agents', hint: 'Open Automations and read the run log.' },
    { action: 'create', label: 'Enable an agent', hint: 'Switch on a Student agent for this school.' },
    { action: 'update', label: 'Run or pause an agent', hint: 'Run an agent, and pause or resume one.' },
    { action: 'delete', label: 'Remove an agent', hint: 'Archive a Student agent.' },
  ];

  return (
    <AiStackCard>
      <AiStackCardHeading
        title="Who may operate Student AI"
