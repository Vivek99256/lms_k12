...
/**
 * Student → AI Stack → Usage & Cost.
 *
 * What the Student module's AI has actually done, and what it cost.
 *
 * ONE CENTRAL USAGE SYSTEM, ASKED A MODULE-SHAPED QUESTION
 *
 * Nothing here is a Student-specific meter. The counts come from
 * `/api/ai/modules/students/usage` — the same endpoint every module's tab calls, over
 * columns the schema already had: `ai_conversations.module_key`, generations through
 * Student templates, reports filed against Students, and the quota on whichever
 * credential Students resolves to. No table, column or counter was added to make this tab
 * work.
 *
 ...
 *
 * Student tool-agent runs live in the Agent Management engine, not in Laravel's
...
export function StudentsUsageCostScreen() {
  ...
  useEffect(() => {
    ...
    fetchModuleUsage(STUDENTS_MODULE)
      .then((next) => {
        if (cancelled) return;
        setUsage(next);
        setError('');
    ...
    fetchRuns({ module: STUDENTS_MODULE, limit: 200 })
      .then((next) => {
  ...
  if (loading && !usage) {
    return <AiStackLoading label="Loading Student AI usage…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={Gauge}
        title="Student AI usage and cost"
        summary="What the Student module consumed, who used it, and what it cost. Every figure below is counted from records — nothing is estimated."
        loading={loading}
        onRefresh={reload}
      />
      ...
      {usage && !usage.module.registered && (
        <AiStackError>
          Students is not registered as an AI module on this estate, so no usage can be attributed to it.
        </AiStackError>
      )}
      ...
      <AiStackMetrics
        metrics={[
          ...
          },
          {
            ...
            hint: runsError ? 'store unreachable' : 'Student tool agents',
          },
          {
            key: 'cost',
            label: 'Cost',
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            ...
            hint="Questions asked from a Student page, from ai_conversations.module_key."
          />
        ...
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="What Students is asking for"
          ...
          {!conversations?.available || !(conversations.turn_detail.by_intent ?? []).length ? (
            <p className="px-5 py-6 text-sm text-slate-500">No questions have been classified for Students yet.</p>
          ) : (
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            ...
            hint="From generations through Student templates, priced at the resolved model's own published rate."
          />
          {!tokens ? (
        ...
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading title="Quota" hint="The daily call ceiling on whichever credential Students resolves to." />
          {!provider?.available ? (
            ...
            <>
              <dl className="divide-y divide-slate-100">
                ...
                <Row
                  label="Binding"
                  value={provider.bound ? 'Students has its own credential' : 'shared with every module'}
                />
                <Row
                  label="Daily limit"
              ...
              {!provider.bound && (
                <p className="border-t border-slate-100 bg-amber-50 px-5 py-3 text-xs leading-5 text-amber-900">
                  Students has no credential of its own, so its calls, quota and spend cannot be separated from every
      ...
      <AiStackCard className="overflow-hidden">
        <AiStackCardHeading
          title="Student tool-agent runs"
          hint="Counted from the Agent Management engine, which is a separate store from the conversation tables above. Runs of the Academic Risk Agent this module is bound to are listed on the Automations tab."
        />
        {runsError ? (
          ...
          <p className="px-5 py-6 text-sm text-slate-500">
            No Student tool agent has been run yet. Enable one on the <strong>Automations</strong> tab.
          </p>
        ) : (
          <>
      ...
      {usage && usage.recent_turns.length > 0 && (
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Recent Student questions"
      ...
      <p className="flex items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-600">
        ...
        <span>
          Conversations, generations and reports are attributed to Students by their own module column. Estate-wide AI
          logs have no module dimension and are deliberately excluded rather than apportioned by guess, so these totals
          are a floor on Student usage, not a share of the whole.
        </span>
      </p>
    </section>
  );
