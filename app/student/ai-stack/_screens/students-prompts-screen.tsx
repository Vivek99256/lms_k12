...
/**
 * Student → AI Stack → Prompts.
 *
 * The prompts behind each Student AI feature, kept out of the code.
 *
 * WHY THIS SITS BESIDE TEMPLATES RATHER THAN INSIDE IT
 *
 * A prompt and a report template are both `ai_templates` rows for
 * `module_key = 'students'`, separated by `kind`. They are not the same thing and must
 ...
 * no model near the figures. That is why a report can print a roll number and a prompt can
 * only describe one. The Templates screen filters to `kind === 'report'` and this one
 ...
 * them all at compile time. What is Student about this one is the scope: the list is
 * fetched with `module_key=students` and every save sets it, so a prompt written here
 * cannot come out filed against another module.
 *
 * PLATFORM PROMPTS ARE NOT EDITED IN PLACE
 *
 * The five Student prompts that ship with the product are platform rows — shared by
...
import { STUDENTS_MODULE, refreshStudentsAiStack } from '@/lib/students/students-ai-stack';
...
function blankForm(options: AiTemplateOptions | null): FormState {
  return {
    ...
    name: '',
    description: '',
    // The two rules every student prompt needs and which are easiest to forget: work
    // only from the enrolment record, and never judge the child from it.
    system_prompt:
      'You answer questions about the school student records. Use only the enrolment records you are given. If a name, a class or a count is not in them, say so rather than estimating. The directory says who is enrolled and where they sit, and nothing about how any of them are doing, so never assess a student for ability, behaviour or character.',
    user_prompt: '',
    status: 'draft',
    category: '',
...
export function StudentsPromptsScreen() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (form.id === null) {
        ...
        setNotice('Student prompt saved.');
      } else {
  ...
  const retire = async (row: AiTemplateRow) => {
    if (!window.confirm(`Retire "${row.name}"? Student AI will stop using it.`)) return;
  ...
  if (loading && rows.length === 0 && !error) {
    return <AiStackLoading label="Loading Student prompts…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        ...
        summary="The text sent to a model behind each Student AI feature. Versioned, editable, and never compiled into the app."
        loading={loading}
        onRefresh={reload}
        actions={
          <button
      ...
      <AiStackHint>
        A prompt is text a model reads; it cannot state a roll number or a count on its own authority. Anything that
        has to print real records belongs on the <strong>Templates</strong> tab, where the figures come from the
        student records rather than a model.
      </AiStackHint>
      ...
      {rows.length === 0 && !loading ? (
        ...
        >
          Student AI features fall back to their built-in wording until a prompt exists here.
        </AiStackEmpty>
      ) : (
        <AiStackCard className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[60rem] border-collapse text-left text-sm">
              <AiStackTableHead
                columns={['Prompt', 'Grounding', 'Version', 'Review', 'In Students', 'Status', 'Updated', 'Actions']}
              />
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              {/* Fixed, and shown rather than hidden: this screen exists to manage
                  Student prompts, and a module selector here would offer one option. */}
              <input
                value="Student"
                readOnly
          ...
          <div className="block">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="students-prompt-description" className="text-sm font-medium text-slate-900">
                What it is for
              </label>
              <AiFieldAssistant
                ...
                entityType="students_ai_prompt"
                related={{ Prompt: form.name }}
              />
            </div>
            <input
              id="students-prompt-description"
              value={form.description}
          ...
          {variables.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Click one to insert it. Green variables carry the student data a grounded answer has to rest on — a
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <label className="block">
              ...
              <input
                ...
                list="students-prompt-categories"
                className="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
              />
              <datalist id="students-prompt-categories">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <label className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900">
              ...
              />
              <span>
                Offer this prompt in the Student AI panel
                <span className="mt-0.5 block text-xs text-slate-500">
                  It appears as a suggestion when somebody opens the assistant from a Student page.
                </span>
              </span>
            </label>
          </div>
