...
/**
 * Student → AI Stack → Knowledge Base.
 *
 * The student material the AI is allowed to draw on when it answers, and proof that each
 * source actually answers.
 *
 * WHAT A "KNOWLEDGE SOURCE" IS HERE
 *
 * Not a folder of uploaded files. For Students it is a read-only MCP tool —
 * `students.directory`, `students.search`, `students.history` — because that is what the
 * assistant really reads when somebody asks a student question. Those tools already carry
 * the tenant scoping, the academic-year filter, the joins onto grade, standard and
 * division, and the rule that a filter naming a class the school does not have returns
 * nothing rather than the whole cohort — all of which the rest of the platform was built
 * on. `ReportDataSourceCatalog` in Laravel lists them from the tool registry itself, so
 * the catalogue on this screen is derived, not maintained: register a student tool in the
 * backend and it appears here with no edit to this file.
 *
 * READ-ONLY BY CONSTRUCTION
 *
 * The backend filters the catalogue on each tool's own `read_only` annotation. A
 * knowledge source that changed records when it was read would mean opening a report
 * altered a child's enrolment.
 *
 * WHY THERE IS A CHECK BUTTON
 *
 * A source can be registered, bound to a template, and still return nothing for this
 * school — wrong academic year, nobody enrolled for it yet, a permission the signed-in
 * user lacks. That is invisible on an inventory and obvious the moment you call it. Check
 * runs the real tool through the existing `/api/mcp/tools/call` proxy, as the signed-in
 * user, and reports what came back. It reads; it writes nothing.
 *
 * NOTHING IS SEEDED INTO THIS TAB. There is no sample document and no fabricated student
 * policy. An estate that has indexed nothing is told exactly that, because a knowledge
 * base padded with invented material is worse than an empty one — it looks authoritative
 * and it is not.
 */
...
import { STUDENTS_MODULE, readStudentsWorkspaceSession } from '@/lib/students/students-ai-stack';
...
export function StudentsKnowledgeBaseScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchTemplateOptions(), fetchTemplates(STUDENTS_MODULE)])
  ...
  /** Student sources only. This screen never lists another module's tools. */
  const sources = useMemo<TemplateDataSource[]>(
    () => (options?.data_sources ?? []).filter((source) => source.module === STUDENTS_MODULE),
    [options],
  );

  /** Which Student templates read each source — the binding, from the template rows. */
  ...
  const check = useCallback(async (source: TemplateDataSource) => {
    ...
    const session = readStudentsWorkspaceSession();
    ...
    try {
      const response = await fetch('/api/mcp/tools/call', {
        ...
        body: JSON.stringify({
          ...
          // than being called wrongly. `students.history` needs a student and will say
          // so — which is a true and useful thing for this tab to show.
          arguments: {},
  ...
  if (loading && !options) {
    return <AiStackLoading label="Loading Student knowledge sources…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        ...
        summary="The student records and documents the AI may draw on when it answers, and whether each one is actually returning data."
        loading={loading}
        onRefresh={reload}
      />
      ...
      <AiStackMetrics
        metrics={[
          { key: 'sources', label: 'Student sources', value: sources.length, hint: 'read-only tools' },
          { key: 'bound', label: 'In use', value: boundCount, hint: 'read by a Student template' },
          { key: 'templates', label: 'Student templates', value: templates.length, hint: 'prompts and reports' },
          {
            key: 'documents',
            label: 'SOP documents',
            value: documentCount ?? '—',
      ...
      <AiStackHint>
        ...
        annotation, so a tool that changes a student record cannot appear here or be bound to a template.
      </AiStackHint>

      {sources.length === 0 ? (
        <AiStackEmpty icon={Database} title="No Student knowledge sources registered">
          The backend reported no read-only student tools. Until one is registered, Student AI has no enrolment
          records to ground an answer in.
        </AiStackEmpty>
      ) : (
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Student data sources"
      ...
      <AiStackCard className="overflow-hidden">
        <AiStackCardHeading
          ...
          hint="SOPs and circulars from ai_sops. Held per institute, not per module — a student-records policy indexed here is visible to the whole estate's AI."
        />
        ...
        {documentsError ? (
          ...
          <p className="px-5 py-6 text-sm text-slate-500">
            No documents are indexed for this institute yet. Student answers therefore rest entirely on the enrolment
...
function summariseMcpPayload(payload: Record<string, unknown> | null): { rows: number | null; detail: string } {
  ...
  const candidates: unknown[] = [
    ...
    // `students.directory` and `students.search` both return their list under
    // `students`. Named explicitly rather than left to the generic keys below, so the
    // count is the cohort and not whichever array happened to be found first —
    // `unresolved_filters` sits beside it and describes the query, not the school.
    (payload.data as Record<string, unknown> | undefined)?.students,
