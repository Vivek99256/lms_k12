...
/**
 * Admission → AI Stack → Activity.
 *
 * The ledger. What happened in Admissions, which AI capability it used, which agent, prompt
 * or template ran, who did it, which enquiry it was about, and how it ended.
 *
 * WHY THIS TAB EXISTS
 *
 ...
 * application. This is the join: a row here names the Admission operation on one side and
 * the AI Stack record on the other, so a question like "which prompt drafted the message we
 * sent that family" has an answer that is a lookup rather than an investigation.
 *
 * WHERE THE ROWS COME FROM
 *
 * `ai_audit_logs`, under an event type of `module.admissions.<operation>` — the same table
 * the agent runs, generation requests and governance refusals already write to. No table
 * was added for this. Each row was written by an Admission screen at the moment it finished
 ...
 *
 * Confirming an admission uses no model. It is recorded anyway, with no artefact against
 * it, because the ledger's job is to show what happened in Admissions — and "this operation
...
export function AdmissionsActivityScreen() {
  ...
  useEffect(() => {
    ...
    fetchModuleActivity(ADMISSIONS_MODULE, { limit: PAGE, operation: operation || undefined })
      .then((next) => {
        if (cancelled) return;
        setActivity(next);
        setError('');
  ...
  if (loading && !activity) {
    return <AiStackLoading label="Loading the Admission AI activity ledger…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={History}
        title="Admission AI activity"
        summary="Every Admission operation the AI Stack recorded — what ran, which capability and record it used, who did it, and how it ended."
        loading={loading}
        onRefresh={reload}
        actions={
          operations.length > 0 ? (
            ...
            >
              ...
              {operations.map(([key, count]) => (
                <option key={key} value={key}>
                  {ADMISSIONS_OPERATIONS[key as AdmissionsOperationKey]?.label ?? key} ({count})
                </option>
              ))}
            </select>
          ) : undefined
      ...
      <AiStackMetrics
        metrics={[
          { key: 'total', label: 'Recorded', value: activity?.total ?? 0, hint: 'Admission operations' },
      ...
      {entries.length === 0 && !loading ? (
        <AiStackEmpty icon={History} title="Nothing recorded yet">
          The ledger fills as people use Admission AI. Build an admission report, run the Admissions Agent, or draft a
          follow-up with AI on an Admission screen, and the operation appears here with whatever AI Stack record it
          used.
        </AiStackEmpty>
      ) : (
                    ...
                    <td className="px-4 py-2.5">
                      <div className="text-xs font-medium text-slate-900">
                        {entry.operation_label ??
                          ADMISSIONS_OPERATIONS[entry.operation as AdmissionsOperationKey]?.label ??
                          entry.operation}
                      </div>
