...
/**
 * Admission → AI Stack → Prompts.
 *
 * The prompts behind each Admission AI feature, kept out of the code.
 *
 * WHY THIS SITS BESIDE TEMPLATES RATHER THAN INSIDE IT
 *
 * A prompt and a report template are both `ai_templates` rows for
 * `module_key = 'admissions'`, separated by `kind`. They are not the same thing and must
 * not share a list: a prompt is text sent to a model, which writes prose; a report is an
 * HTML layout whose placeholders are filled by substitution from rows a tool fetched, with
 * no model near the figures. That is why a report can print an enquiry number and a prompt
 * can only describe one. The Templates screen filters to `kind === 'report'` and this one
 * to `kind === 'prompt'`, so an author editing "the follow-up prompt" cannot land in the
 * report layout by accident.
 *
 * DECENTRALISED, SAME STORE
 *
 * This calls the same `lib/intelligence/ai-templates` client the central console and the
 * Fees and Attendance screens call, so the contract is shared and a change to it breaks
 * them all at compile time. What is Admission about this one is the scope: the list is
 * fetched with `module_key=admissions` and every save sets it, so a prompt written here
 * cannot come out filed against another module.
 *
 * PLATFORM PROMPTS ARE NOT EDITED IN PLACE
 *
 * The five Admission prompts that ship with the product are platform rows — shared by
...
import { ADMISSIONS_MODULE, refreshAdmissionsAiStack } from '@/lib/admissions/admissions-ai-stack';
...
function blankForm(options: AiTemplateOptions | null): FormState {
  return {
    ...
    name: '',
    description: '',
    // The two rules every admission prompt needs and which are easiest to forget: work
    // only from the enquiry record, and never state an outcome the school has not decided.
    system_prompt:
      'You answer questions about school admissions. Use only the enquiry and registration records you are given. If a figure or a date is not in them, say so rather than estimating. An enquiry is a family that has asked about a place — the record holds no decision, so never state or imply that a place has been offered, held or refused.',
    user_prompt: '',
    status: 'draft',
    category: '',
...
export function AdmissionsPromptsScreen() {
    ...
    try {
      setPreview(
        await previewTemplate({
          ...
          // Sample enquiries rather than the endpoint's default sample fees. An author
          // cannot tell whether an admission placeholder landed when every sample row is
          // a fee, and a fee on this screen is the one kind of data that must not be here.
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (form.id === null) {
        ...
        setNotice('Admission prompt saved.');
      } else {
  ...
  const retire = async (row: AiTemplateRow) => {
    if (!window.confirm(`Retire "${row.name}"? Admission AI will stop using it.`)) return;
  ...
  if (loading && rows.length === 0 && !error) {
    return <AiStackLoading label="Loading Admission prompts…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={Terminal}
        title="Admission prompts"
        summary="The text sent to a model behind each Admission AI feature. Versioned, editable, and never compiled into the app."
        loading={loading}
        onRefresh={reload}
        actions={
          <button
      ...
      <AiStackHint>
        A prompt is text a model reads; it cannot state an enquiry number or a count on its own authority. Anything that
        has to print real records belongs on the <strong>Templates</strong> tab, where the figures come from the
        admission records rather than a model.
      </AiStackHint>
      ...
      {rows.length === 0 && !loading ? (
        <AiStackEmpty
          icon={Terminal}
          title="No Admission prompts yet"
          action={
            <button
              type="button"
        ...
        >
          Admission AI features fall back to their built-in wording until a prompt exists here.
        </AiStackEmpty>
      ) : (
        <AiStackCard className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[60rem] border-collapse text-left text-sm">
              <AiStackTableHead
                columns={['Prompt', 'Grounding', 'Version', 'Review', 'In Admissions', 'Status', 'Updated', 'Actions']}
              />
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-base font-semibold text-slate-950">
              {form.id === null
                ? 'New Admission prompt'
                : form.forks
                  ? 'Customise for this institute'
                  : 'Edit Admission prompt'}
            </h3>
            <button
              type="button"
              onClick={() => {
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              {/* Fixed, and shown rather than hidden: this screen exists to manage
                  Admission prompts, and a module selector here would offer one option. */}
              <input
                value="Admission"
                readOnly
          ...
          <div className="block">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="admissions-prompt-description" className="text-sm font-medium text-slate-900">
                What it is for
              </label>
              <AiFieldAssistant
                ...
                entityType="admissions_ai_prompt"
                related={{ Prompt: form.name }}
              />
            </div>
            <input
              id="admissions-prompt-description"
              value={form.description}
          ...
          {variables.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Click one to insert it. Green variables carry the admission data a grounded answer has to rest on — a
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <label className="block">
              ...
              <input
                ...
                list="admissions-prompt-categories"
                className="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
              />
              <datalist id="admissions-prompt-categories">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <label className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900">
              ...
              />
              <span>
                Offer this prompt in the Admission AI panel
                <span className="mt-0.5 block text-xs text-slate-500">
                  It appears as a suggestion when somebody opens the assistant from an Admission page.
                </span>
              </span>
            </label>
          </div>
