...
/**
 * Admission → AI Stack → Usage & Cost.
 *
 * What the Admission module's AI has actually done, and what it cost.
 *
 * ONE CENTRAL USAGE SYSTEM, ASKED A MODULE-SHAPED QUESTION
 *
 * Nothing here is an Admission-specific meter. The counts come from
 * `/api/ai/modules/admissions/usage` — the same endpoint every module's tab calls, over
 * columns the schema already had: `ai_conversations.module_key`, generations through
 * Admission templates, reports filed against Admissions, and the quota on whichever
 * credential Admissions resolves to. No table, column or counter was added to make this tab
 * work.
 *
 ...
 * every rate null because a guessed rate is worse than no rate on a screen somebody uses to
 * explain a bill. So when it cannot multiply, this screen shows no money and prints the
 ...
 *
 * Admission tool-agent runs live in the Agent Management engine, not in Laravel's
...
export function AdmissionsUsageCostScreen() {
  ...
  useEffect(() => {
    ...
    fetchModuleUsage(ADMISSIONS_MODULE)
      .then((next) => {
        if (cancelled) return;
        setUsage(next);
        setError('');
    ...
    fetchRuns({ module: ADMISSIONS_MODULE, limit: 200 })
      .then((next) => {
  ...
  if (loading && !usage) {
    return <AiStackLoading label="Loading Admission AI usage…" />;
  }
  ...
  return (
    <section className="space-y-5">
      <AiStackHeader
        icon={Gauge}
        title="Admission AI usage and cost"
        summary="What the Admission module consumed, who used it, and what it cost. Every figure below is counted from records — nothing is estimated."
        loading={loading}
        onRefresh={reload}
      />
      ...
      {usage && !usage.module.registered && (
        <AiStackError>
          Admissions is not registered as an AI module on this estate, so no usage can be attributed to it.
        </AiStackError>
      )}
      ...
      <AiStackMetrics
        metrics={[
          ...
          },
          {
            ...
            hint: 'asked an Admission question',
          },
          {
            key: 'latency',
          ...
          },
          {
            ...
            hint: runsError ? 'store unreachable' : 'Admission tool agents',
          },
          {
            key: 'cost',
            label: 'Cost',
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            ...
            hint="Questions asked from an Admission page, from ai_conversations.module_key."
          />
        ...
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="What Admissions is asking for"
          ...
          {!conversations?.available || !(conversations.turn_detail.by_intent ?? []).length ? (
            <p className="px-5 py-6 text-sm text-slate-500">No questions have been classified for Admissions yet.</p>
          ) : (
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            ...
            hint="From generations through Admission templates, priced at the resolved model's own published rate."
          />
          {!tokens ? (
        ...
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading title="Quota" hint="The daily call ceiling on whichever credential Admissions resolves to." />
          {!provider?.available ? (
            ...
            <>
              <dl className="divide-y divide-slate-100">
                ...
                <Row
                  label="Binding"
                  value={provider.bound ? 'Admissions has its own credential' : 'shared with every module'}
                />
                <Row
                  label="Daily limit"
              ...
              {!provider.bound && (
                <p className="border-t border-slate-100 bg-amber-50 px-5 py-3 text-xs leading-5 text-amber-900">
                  Admissions has no credential of its own, so its calls, quota and spend cannot be separated from every
      ...
      <AiStackCard className="overflow-hidden">
        <AiStackCardHeading
          title="Admission tool-agent runs"
          hint="Counted from the Agent Management engine, which is a separate store from the conversation tables above. Runs of the backend Admissions Agent are listed on the Automations tab."
        />
        {runsError ? (
          ...
          <p className="px-5 py-6 text-sm text-slate-500">
            No Admission tool agent has been run yet. Enable one on the <strong>Automations</strong> tab.
          </p>
        ) : (
          <>
      ...
      {usage && usage.recent_turns.length > 0 && (
        <AiStackCard className="overflow-hidden">
          <AiStackCardHeading
            title="Recent Admission questions"
      ...
      <p className="flex items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-600">
        ...
        <span>
          Conversations, generations and reports are attributed to Admissions by their own module column. Estate-wide AI
          logs have no module dimension and are deliberately excluded rather than apportioned by guess, so these totals
          are a floor on Admission usage, not a share of the whole.
        </span>
      </p>
    </section>
  );
