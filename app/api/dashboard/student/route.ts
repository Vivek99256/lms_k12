...
/**
 * Server-side proxy for the Student dashboard aggregate endpoint.
 *
 ...
 *
 * Upstream: POST {LARAVEL_BASE_URL}/api/student-dashboard/summary
 *   next_lms_erp/routes/api.php → RoleDashboardApiController::studentSummary
 ...
 */
const LARAVEL_PATH = '/api/student-dashboard/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the student dashboard proxy.', 400);
  }
  if (!token) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the student dashboard request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the student dashboard request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel student dashboard endpoint.',
      502
    );
  }
}
