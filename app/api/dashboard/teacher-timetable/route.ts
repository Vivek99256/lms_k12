...
/**
 * Server-side proxy for the Teacher "My timetable" endpoint.
 *
 ...
 *
 * Upstream: POST {LARAVEL_BASE_URL}/api/teacher-timetable/summary
 *   next_lms_erp/routes/api.php → TeacherTimetableApiController::summary
 ...
 */
const LARAVEL_PATH = '/api/teacher-timetable/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the teacher timetable proxy.', 400);
  }
  if (!token) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the teacher timetable request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the teacher timetable request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel teacher timetable endpoint.',
      502
    );
  }
}
