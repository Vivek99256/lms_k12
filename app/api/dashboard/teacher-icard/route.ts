...
/**
 * Server-side proxy for the self-service "My ID card" endpoint.
 *
 * Browser → this route → Laravel. Structurally identical to
 * app/api/dashboard/teacher/route.ts (session travels as `x-*` headers,
 * same error-shape helpers) — this just points at a different upstream path.
 *
 * Upstream: POST {LARAVEL_BASE_URL}/api/teacher-icard/mine
 *   next_lms_erp/routes/api.php → TeacherIcardApiController::mine()
 *   Route sits behind `api.session` (ApiSessionHydrator) — the JWT alone
 *   decides the caller's identity; the Laravel controller scopes the card
 *   query to session()->get('user_id') only and returns 403 for students.
 */
const LARAVEL_PATH = '/api/teacher-icard/mine';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the teacher I-card proxy.', 400);
  }
  if (!token) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the teacher I-card request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the teacher I-card request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel teacher I-card endpoint.',
      502
    );
  }
}
