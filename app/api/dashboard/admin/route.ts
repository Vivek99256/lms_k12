...
/**
 * Server-side proxy for the Admin dashboard aggregate endpoint.
 *
 ...
 *
 * Upstream: POST {LARAVEL_BASE_URL}/api/admin-dashboard/summary
 *   next_lms_erp/routes/api.php → RoleDashboardApiController::adminSummary
 ...
 *   non-admin token.
 */
const LARAVEL_PATH = '/api/admin-dashboard/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the admin dashboard proxy.', 400);
  }
  if (!token) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the admin dashboard request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the admin dashboard request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel admin dashboard endpoint.',
      502
    );
  }
}
