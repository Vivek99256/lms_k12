...
/**
 * Server-side proxy for the Teacher "fee dues for my class" aggregate.
 *
 * Browser → this route → Laravel. Same shape as
 * app/api/dashboard/teacher/route.ts (session travels as `x-*` headers),
 * which is how this module avoids browser→Laravel CORS.
 *
 * Upstream: POST {LARAVEL_BASE_URL}/api/teacher-fee-dues/summary
 *   next_lms_erp/routes/api.php → TeacherFeeDuesApiController::summary
 ...
 */
const LARAVEL_PATH = '/api/teacher-fee-dues/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the teacher fee dues proxy.', 400);
  }
  if (!token) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the teacher fee dues request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the teacher fee dues request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel teacher fee dues endpoint.',
      502
    );
  }
}
