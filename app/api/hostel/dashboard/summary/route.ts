...
/**
 * Server-side proxy for the Hostel dashboard aggregate endpoint.
 * Same convention as app/api/fees/dashboard/summary/route.ts.
 *
 * Upstream:
 *   POST {LARAVEL_BASE_URL}/api/hostel-dashboard/summary
 *   next_lms_erp/routes/api.php → HostelDashboardApiController::summary
 */
const LARAVEL_PATH = '/api/hostel-dashboard/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the hostel dashboard proxy.', 400);
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the hostel dashboard request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the hostel dashboard request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel hostel dashboard endpoint.',
      502
    );
  }
}
