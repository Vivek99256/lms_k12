...
/**
 * Server-side proxy for the Admissions dashboard aggregate endpoint.
 *
 * Browser → this route → Laravel. Same convention as the Fees dashboard proxy
 * (app/api/fees/dashboard/summary/route.ts) — session travels as `x-*`
 * headers, forwarded as a JSON body because the Laravel route accepts JSON
 * and runs no session middleware (the controller seeds session from the
 * body itself).
 *
 * Upstream:
 *   POST {LARAVEL_BASE_URL}/api/admissions-dashboard/summary
 *   next_lms_erp/routes/api.php → AdmissionsDashboardApiController::summary
 */
const LARAVEL_PATH = '/api/admissions-dashboard/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the admissions dashboard proxy.', 400);
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the admissions dashboard request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the admissions dashboard request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel admissions dashboard endpoint.',
      502
    );
  }
}
