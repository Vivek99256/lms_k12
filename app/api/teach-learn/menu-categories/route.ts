...
/**
 * Server-side proxy for the Teach/Learn category navigation feed.
 *
 * Browser → this route → Laravel, the same shape as
 * app/api/fees/menu-categories/route.ts, which is how the fees module keeps
 * Laravel off the browser's origin and avoids CORS.
 *
 * Upstream:
 *   GET {LARAVEL_BASE_URL}/api/teach-learn/menu-categories
 *   next_lms_erp/routes/api.php → TeachLearnMenuCategoryApiController::index
 *
 ...
 * this module already sends on every teach/learn request.
 */
const LARAVEL_PATH = '/api/teach-learn/menu-categories';
...
export async function GET(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the teach/learn menu-categories proxy.', 400);
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the teach/learn menu-categories request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the teach/learn menu-categories request.', 502, {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      return jsonError('The Laravel teach/learn menu-categories request timed out.', 504);
    }

    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel teach/learn menu-categories endpoint.',
      502
    );
  }
}
