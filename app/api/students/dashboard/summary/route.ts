...
/**
 * Server-side proxy for the Students dashboard aggregate endpoint.
 *
 * Browser → this route → Laravel. Same convention as the Fees dashboard proxy
 * (app/api/fees/dashboard/summary/route.ts).
 *
 * Upstream:
 *   POST {LARAVEL_BASE_URL}/api/students-dashboard/summary
 *   next_lms_erp/routes/api.php → StudentsDashboardApiController::summary
 */
const LARAVEL_PATH = '/api/students-dashboard/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the students dashboard proxy.', 400);
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the students dashboard request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the students dashboard request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel students dashboard endpoint.',
      502
    );
  }
}
