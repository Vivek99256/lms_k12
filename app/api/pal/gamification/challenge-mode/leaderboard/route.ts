...
export async function GET(request: NextRequest) {
    try {
        ...
        if (Number.isNaN(challengeId) || !weekStart) {
            return NextResponse.json({ status: '0', message: 'challenge_id and week_start are required.' }, { status: 400 });
        }

        const weekStartDate = new Date(weekStart);
        if (Number.isNaN(weekStartDate.getTime())) {
            return NextResponse.json({ status: '0', message: 'Invalid week_start date. Use YYYY-MM-DD format.' }, { status: 400 });
        }
        ...
        try {
            ...
            const entries = await service.getWeeklyLeaderboard(challengeId, weekStartDate, ctx);
