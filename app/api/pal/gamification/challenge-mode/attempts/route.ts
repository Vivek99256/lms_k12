...
import { validateCmSession } from '../../challenge-mode/_auth';
...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            ...
            const attempts = await service.getStudentChallengeHistory(ctx);

            try { await store.close(); } catch { /* ignore */ }

            return NextResponse.json({ status: '1', data: { attempts } });
        } catch (storeErr) {
        ...
        const message = err instanceof Error ? err.message : 'Unable to load challenge history.';
