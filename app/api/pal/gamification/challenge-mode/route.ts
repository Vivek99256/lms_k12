...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            ...
            const status = await service.getStudentChallengeStatus(ctx);
        ...
        const message = err instanceof Error ? err.message : 'Unable to load challenge mode status.';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const isOptedIn = body.isOptedIn === true;
        ...
        try {
            const service = createPalCmService(store);
            const optIn = await service.setChallengeOptIn(ctx.user_id.toString(), isOptedIn, ctx);

            try { await store.close(); } catch { /* ignore */ }

            return NextResponse.json({ status: '1', data: { optIn } });
        } catch (storeErr) {
            if (!isDbError(storeErr)) throw storeErr;
            useMock = true;
        }
        ...
        const message = err instanceof Error ? err.message : 'Unable to update opt-in status.';
        const status = message === 'Unauthorized' || message === 'Missing user context' ? 401 : 500;
        return NextResponse.json({ status: '0', message }, { status });
    }
}
