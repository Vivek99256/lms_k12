...
export async function validateCqSession(request: NextRequest): Promise<{ userId: string; subInstituteId: string; syear: string; isStaff: boolean; isBearer: boolean } | null> {
...
export function readGrade(request: NextRequest): number | null {
  const gradeStr = String(new URL(request.url).searchParams.get('grade') || '').trim();
