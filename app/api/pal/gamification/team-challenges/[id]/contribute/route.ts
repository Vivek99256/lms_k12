...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (session.isBearer && session.isStaff) {
            return NextResponse.json({ status: '0', message: 'Teachers cannot submit contributions.' }, { status: 403 });
        }
        ...
        const input = {
            event_type: String(body.event_type || 'mastery'),
            source_id: body.source_id ? String(body.source_id) : null,
            value: body.value !== undefined ? Number(body.value) : 1,
            idempotency_key: String(body.idempotency_key || ''),
        };

        if (!input.idempotency_key) {
            return NextResponse.json({ status: '0', message: 'idempotency_key is required.' }, { status: 400 });
        }
        ...
        try {
            ...
            const progress = await service.submitContribution(challengeId, input as unknown as Parameters<typeof service.submitContribution>[1], ctx);
            ...
            if (!progress) {
                return NextResponse.json({ status: '0', message: 'Cannot submit contribution. Challenge not found or not joinable.' }, { status: 400 });
            }

            return NextResponse.json({ status: '1', data: { progress } });
        } catch (storeErr) {
