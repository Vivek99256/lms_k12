...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (session.isBearer && session.isStaff) {
            return NextResponse.json({ status: '0', message: 'Teachers cannot join challenges.' }, { status: 403 });
        }
        ...
        try {
            ...
            const joined = await service.joinChallenge(challengeId, ctx);
            ...
            return NextResponse.json({ status: '1', data: { joined } });
        } catch (storeErr) {
        ...
        const message = err instanceof Error ? err.message : 'Unable to join challenge.';
