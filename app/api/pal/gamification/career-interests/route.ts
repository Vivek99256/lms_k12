...
import type { InterestDeclarationInput } from '@/app/pal/data/cq-types';
...
export async function POST(request: NextRequest) {
    ...
    const interestType = String(body.interest_type ?? body.interestType ?? '').trim().toLowerCase();
    const interestValue = String(body.interest_value ?? body.interestValue ?? '').trim();

    if (!interestType || !interestValue) {
      return NextResponse.json(
        { status: '0', message: 'interest_type and interest_value are required.' },
        { status: 400 }
      );
    }

    const validTypes = ['riasec', 'pathway', 'skill', 'cluster'];
    if (!validTypes.includes(interestType)) {
      return NextResponse.json(
        { status: '0', message: `Invalid interest_type. Allowed: ${validTypes.join(', ')}.` },
        { status: 400 }
      );
    }

    const input: InterestDeclarationInput = {
      interestType: interestType as InterestDeclarationInput['interestType'],
      interestValue,
    ...
    try {
      ...
      const interest = await service.declareInterest(ctx.userId, ctx.subInstituteId, ctx.syear, grade, input);
    ...
    const message = err instanceof Error ? err.message : 'Unable to save interest declaration.';
