...
/**
 * Server-side proxy for the Transportation dashboard aggregate endpoint.
 * Same convention as app/api/fees/dashboard/summary/route.ts.
 *
 * Upstream:
 *   POST {LARAVEL_BASE_URL}/api/transportation-dashboard/summary
 *   next_lms_erp/routes/api.php → TransportationDashboardApiController::summary
 */
const LARAVEL_PATH = '/api/transportation-dashboard/summary';
...
export async function POST(request: NextRequest) {
  ...
  if (!baseUrl) {
    return jsonError('Laravel base URL is missing for the transportation dashboard proxy.', 400);
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return jsonError('Laravel redirected the transportation dashboard request instead of returning JSON.', 502, {
    ...
    try {
      ...
      return jsonError('Laravel returned a non-JSON response for the transportation dashboard request.', 502, {
    ...
    return jsonError(
      error instanceof Error ? error.message : 'Unable to connect to the Laravel transportation dashboard endpoint.',
      502
    );
  }
}
