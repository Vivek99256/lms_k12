...
import type {
  AlignmentPayload, AlignmentStatus, CareerEvidencePayload, CareerEvidenceStatus,
  EvidenceLevel,
} from '../_lib/types';
      ...
      <CardContent>
        ...
        <div className="mt-2 divide-y">
          {COUNSELLOR_ACTIONS.map((action) => {
            // "Explore Adjacent Careers" is a label only — the actual
            // ranked occupation matches live in the Match Profile tab,
            // so clicking it here has no destination. Render it as a
            // non-interactive row rather than a button.
            const nonInteractive = action.type === 'explore_adjacent';
            const body = (
              <>
            ...
            if (nonInteractive) {
              return (
                <div key={action.type} className="flex w-full items-center gap-3 py-2.5" aria-disabled>
                  {body}
                </div>
              );
            }
...
/**
 * "Explore Adjacent Careers" (Counsellor Actions) used to surface a
 * knowledge-based panel here, but those ranked occupation matches now
 * live in the Match Profile tab's Matching Occupations section. The
 * Counsellor Action row remains as a non-interactive label for
 * historical continuity, with no on-click destination.
 */
...
export function CareerIntelligence({ studentId }: { studentId?: string }) {
  ...
  const onSelectAction = useCallback((type: string) => {
    // 'explore_adjacent' is intentionally not handled here — the
    // Counsellor Actions row is rendered as a non-interactive label,
    // because the actual ranked occupation matches live in the Match
    // Profile tab's Matching Occupations section, not here.
    if (type === 'explore_adjacent') return;
