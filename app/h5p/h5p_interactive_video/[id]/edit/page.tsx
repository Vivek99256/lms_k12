...
/**
 * Interactive video edit — mirrors Laravel `GET /h5p/h5p_interactive_video/{id}/edit`
 * (resources/views/lms/h5p/interactiveVideo/edit.blade.php). The update
 * REPLACES the whole interactions set server-side.
 */
...
function normalizeType(type: string | null | undefined): VideoInteractionType {
  return type === 'true_false' || type === 'text_input' ? type : 'multiple_choice';
}

function normalizeTrueFalseCorrect(value: string | null | undefined): string {
  const normalized = String(value ?? '').trim().toLowerCase();
  if (normalized === 'true' || normalized === '1') return '1';
  if (normalized === 'false' || normalized === '2' || normalized === '0') return '2';
  return '';
}

function interactionToRow(interaction: H5pVideoInteraction): InteractionRow {
  const type = normalizeType(interaction.interaction_type);
  let options: string[] = [];
  let correct = '';
  if (type === 'multiple_choice') {
    options = parseInteractionOptions(interaction).map((option) => option.label);
    while (options.length < 4) options.push('');
    correct = interaction.correct_answer !== null && interaction.correct_answer !== undefined
      ? String(interaction.correct_answer)
      : '';
  } else if (type === 'true_false') {
    correct = normalizeTrueFalseCorrect(interaction.correct_answer);
  } else {
    correct = interaction.correct_answer !== null && interaction.correct_answer !== undefined
      ? String(interaction.correct_answer)
      : '';
  ...
  return {
    time: String(interaction.time ?? 0),
    type,
...
function InteractiveVideoEditContent() {
  ...
  useEffect(() => {
    ...
    fetchVideo(videoId, ctx)
      .then((loaded) => {
        ...
        const loadedRows = (loaded.interactions ?? []).map(interactionToRow);
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await updateVideo(video.id, ctx, {
        title: title.trim(),
        interactions,
        videoFile: file,
        existingVideoPath: video.video_path ?? '',
      });
    ...
    <div className="flex-1 overflow-auto p-4 sm:p-6">
      <div className="mx-auto max-w-5xl">
        ...
        {!hasH5pContext(ctx) ? (
          ...
          <InlineBanner kind="error" message="Interactive video not found." />
        ) : (
          <>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              <div className="grid gap-4">
                ...
                <div className="grid gap-1.5">
                  <Label className="text-slate-700">Current video</Label>
                  {video.video_path ? (
                    <video controls className="w-full rounded-xl" src={video.video_path} />
                  ) : (
                    <p className="text-xs text-slate-500">No video file on record.</p>
                  )}
                </div>
                ...
                <div className="grid gap-1.5">
                  ...
                  <p className="text-xs text-slate-500">
                    Allowed: mp4, mov, avi, mkv, webm. Maximum size 500MB. Leave empty to keep the current video.
                  </p>
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <p className="mb-4 text-xs text-slate-500">
                Saving replaces the full set of interactions for this video.
              </p>
...
export default function InteractiveVideoEditPage() {
  return (
    <Suspense fallback={<LoadingState label="Loading…" />}>
      <InteractiveVideoEditContent />
    </Suspense>
  );
}
