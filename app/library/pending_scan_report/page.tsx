...
function printRows(rows: Row[]) {
  const html = `<html><head><title>Pending Scan Report</title><style>
...
export default function PendingScanReportPage() {
  ...
  const handleSearch = async () => {
    ...
    setMessage(null);
    try {
      const params = new URLSearchParams({ path: 'verified_book_report_pending' });
      ...
      setRows([]);
      setMessage({ type: 'error', text: error instanceof Error ? error.message : 'Unable to load pending scan report.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageFrame>
      <PageHeader
        title="Pending Scan Report"
        description="Review item codes that have not yet been verified in the scan workflow."
        action={<div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" onClick={() => {
            if (exportRows.length === 0) return;
            const headers = Object.keys(exportRows[0]);
            const csv = [headers.join(','), ...exportRows.map((row) => headers.map((header) => escapeCsv(row[header] ?? '')).join(','))].join('\n');
            downloadFile('pending-scan-report.csv', csv, 'text/csv;charset=utf-8;');
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <Table className="min-w-[760px]">
              <TableHeader><TableRow className="bg-slate-100 hover:bg-slate-100"><TableHead>Sr No</TableHead><TableHead>SYear</TableHead><TableHead>Item Code</TableHead><TableHead>Title</TableHead><TableHead>Collection Type</TableHead></TableRow></TableHeader>
              <TableBody>
                {loading ? <LoadingRows colSpan={5} label="Loading pending scan report" /> : filteredRows.length > 0 ? filteredRows.map((row, index) => (
                  <TableRow key={`${row.itemCode}-${index}`} className="odd:bg-white even:bg-slate-50/60">
                    <TableCell>{index + 1}</TableCell><TableCell>{row.syear || '-'}</TableCell><TableCell>{row.itemCode || '-'}</TableCell><TableCell>{row.title || '-'}</TableCell><TableCell>{row.collectionType || '-'}</TableCell>
                  </TableRow>
                )) : <EmptyTableRow colSpan={5} label="No pending scan rows available." />}
              </TableBody>
            </Table>
          </div>
        </div>
