...
/**
 * Shared transport for the legacy Laravel controllers.
 *
 * Screens talk to the Blade-era controllers through the existing
 * `/api/proxy` (JSON) and `/api/proxy-file` (multipart) routes with
...
type RequestOptions = {
  ...
  /**
   * Some legacy controllers insert `$request->except([...])` straight into a
   * table, so the API context fields must stay out of the body. Set this to
   * send only the explicit values.
   */
  bareBody?: boolean;
  /**
   * The `adminapi` endpoints answer an empty result set with `status: 0` and
   * "No Record" rather than an empty array. Set this so that reads treat it as
   * "nothing found" instead of a failure.
   */
  tolerateStatusZero?: boolean;
};
...
function failureFrom(payload: unknown, status: number): string {
  ...
  if (status === 419) {
    return 'The ERP rejected the request (CSRF token mismatch). This endpoint is not exposed as a stateless API yet.';
  }
...
export async function legacyRequest(
  ...
  const payloadBody = options.body
    ? JSON.stringify(options.bareBody ? options.body : contextBody(session, options.body))
    : undefined;
  ...
  const response = await fetch(
    `/api/proxy?path=${encodeURIComponent(path)}&${params.toString()}`,
    {
      method,
      cache: 'no-store',
      headers: createAuthHeaders(session, payloadBody ? 'application/json' : undefined),
  ...
  const failureStatuses = options.tolerateStatusZero ? ['2'] : ['0', '2'];
  if (isRecord(payload) && failureStatuses.includes(normalizeApiStatus(payload as ApiEnvelope))) {
    throw new Error(messageFrom(payload, 'The request could not be completed.'));
  }
  return isRecord(payload) ? payload : {};
}

/** Multipart writes (photo / bill / attachment uploads) stream through the file proxy. */
