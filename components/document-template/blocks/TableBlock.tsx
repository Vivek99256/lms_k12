    ...
    return (
        ...
        >
            <div
                ...
                className={`relative ${editMode ? "ring-2 ring-blue-400/60 ring-offset-1" : ""}`}
                style={{
            ...
            >
                ...
                {/* Edit mode indicator */}
                {editMode && (
                    <div className="absolute -top-6 left-0 bg-[#0D6EFD] text-white text-[10px] font-semibold px-2 py-0.5 rounded-t-md shadow-sm z-50 pointer-events-none select-none">
                ...
                {editMode && editor && (
                    <div
                        className="absolute -bottom-8 left-0 right-0 flex items-center gap-0.5 bg-white border border-slate-200 rounded-b-lg shadow-lg px-1 py-0.5 z-50"
                    ...
                    >
                        {[
                            ...
                            <button key={a.label} className="px-1.5 py-0.5 text-[10px] font-medium rounded hover:bg-blue-100 text-slate-700 whitespace-nowrap" onClick={a.fn}>{a.label}</button>
                        ))}
                        ...
                        <div className="w-px h-3.5 bg-slate-200" />
                        <button className="px-1.5 py-0.5 text-[10px] font-medium rounded hover:bg-blue-100 text-slate-700 whitespace-nowrap" onClick={() => editor.chain().focus().toggleHeaderRow().run()}>Header</button>
                        <button className="px-1.5 py-0.5 text-[10px] font-medium rounded hover:bg-blue-100 text-slate-700 whitespace-nowrap" onClick={() => editor.chain().focus().mergeCells().run()} title="Select cells first (Shift+Click or drag), then merge">Merge</button>
                        <button className="px-1.5 py-0.5 text-[10px] font-medium rounded hover:bg-blue-100 text-slate-700 whitespace-nowrap" onClick={() => editor.chain().focus().splitCell().run()}>Split</button>
                    </div>
                )}
                        ...
                        >
                            <div className="flex h-9 items-center gap-0.5 px-1">
                                <select
                                    ...
                                    className="h-9 px-3 text-[14px] font-semibold border border-slate-200 outline-none bg-white hover:bg-slate-50 rounded-lg transition-colors cursor-pointer w-32 truncate"
                                >
                            ...
                            <div className="flex h-9 items-center gap-0.5 px-1 rounded-lg border border-slate-200 bg-white">
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); applyFontSize(getToolbarFontSize() - 1); }} className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-slate-100 text-slate-700 hover:text-slate-950 font-bold transition-colors">-</button>
                            ...
                            <div className="flex h-9 items-center gap-0.5 px-1">
                                <div className="relative flex items-center justify-center h-9 min-w-9 rounded-lg hover:bg-slate-100 cursor-pointer overflow-hidden transition-colors group" title="Text Color">
                                    <span className="font-bold text-slate-800 text-[15px] pointer-events-none pb-1 border-b-4 group-hover:border-b-8 transition-all" style={{ borderColor: editor.getAttributes("textStyle").color || "#000000" }}>A</span>
                            ...
                            <div className="flex h-9 items-center gap-0.5 px-1">
                                <div className="relative flex items-center justify-center h-9 min-w-9 rounded-lg hover:bg-slate-100 cursor-pointer overflow-hidden transition-colors group" title="Highlight Color">
                                    <span className="font-bold text-slate-800 text-xs pointer-events-none pb-1 border-b-4 group-hover:border-b-8 transition-all" style={{ borderColor: editor.getAttributes("highlight").color || "transparent", backgroundColor: editor.getAttributes("highlight").color || "transparent" }}>H</span>
                            ...
                            <div className="flex h-9 items-center gap-0.5 px-1">
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleBold().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("bold") ? "bg-[#0D6EFD] text-white" : ""}`} title="Bold"><Bold className="h-5 w-5" /></button>
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleItalic().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("italic") ? "bg-[#0D6EFD] text-white" : ""}`} title="Italic"><Italic className="h-5 w-5" /></button>
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleUnderline().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("underline") ? "bg-[#0D6EFD] text-white" : ""}`} title="Underline"><UnderlineIcon className="h-5 w-5" /></button>
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleStrike().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("strike") ? "bg-[#0D6EFD] text-white" : ""}`} title="Strikethrough"><Strikethrough className="h-5 w-5" /></button>
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); toggleTextCase(); }} className="h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors" title="Toggle case"><CaseSensitive className="h-5 w-5" /></button>
                            </div>
                            ...
                            <div className="flex h-9 items-center gap-0.5 px-1">
                                {[
                                    ...
                                    <button key={align} type="button" onMouseDown={(e) => { e.preventDefault(); setAlignment(align); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive({ textAlign: align }) ? "bg-[#0D6EFD] text-white" : ""}`} title={`Align ${align}`}>
                            ...
                            <div className="flex h-9 items-center gap-0.5 px-1">
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleBulletList().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("bulletList") ? "bg-[#0D6EFD] text-white" : ""}`} title="Bullet list">
                                    <List className="h-5 w-5" />
                                </button>
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleOrderedList().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("orderedList") ? "bg-[#0D6EFD] text-white" : ""}`} title="Numbered list">
                                    <ListOrdered className="h-5 w-5" />
                                </button>
                                <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().toggleBlockquote().run(); }} className={`h-9 min-w-9 inline-flex items-center justify-center rounded-lg px-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 transition-colors ${editor.isActive("blockquote") ? "bg-[#0D6EFD] text-white" : ""}`} title="Quote">
    ...
    if (!isDesignTab) {
        return (
            <div className="flex flex-col space-y-4">
                <p className="text-sm text-muted-foreground bg-blue-50 text-[#0D6EFD] p-2 rounded">
                ...
                <div className="flex flex-col space-y-1">
                    <label className="text-xs font-medium text-slate-600">Table Width (px)</label>
                ...
                <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="headerRowToggle" className="text-xs text-slate-600">Header Row</label>
                </div>

                {/* ── STRUCTURE PANEL ──────────────────────────── */}
                <div className="border-t pt-3">
                    <div className="text-xs font-bold text-[#0D6EFD] uppercase tracking-wider mb-2">Table Structure</div>
                    <div className="grid grid-cols-2 gap-2">
                        {editor ? (
                            ...
                            <p className="text-[10px] text-slate-400 col-span-2">Enter edit mode to use structure controls</p>
                        )}
                    </div>
                </div>
                ...
                {/* ── MERGE / SPLIT ────────────────────────────── */}
                <div className="border-t pt-3">
                    <div className="text-xs font-bold text-[#0D6EFD] uppercase tracking-wider mb-2">Merge / Split</div>
                    ...
                    <div className="flex gap-2">
                        {editor ? (
                            ...
                            <p className="text-[10px] text-slate-400">Enter edit mode first</p>
                        )}
                    </div>
                </div>
                ...
                {/* ── PER-CELL BACKGROUND ──────────────────────── */}
                <div className="border-t pt-3">
                    <div className="text-xs font-bold text-[#0D6EFD] uppercase tracking-wider mb-2">Cell Background Color</div>
                    {currentKey ? (
                        <div className="space-y-1">
                            <p className="text-[10px] text-slate-500">Cell: Row {cellPos!.row + 1}, Col {cellPos!.col + 1}</p>
                            <CellColorPicker
                        ...
                        <p className="text-[10px] text-slate-400">Click a cell in edit mode to select it, then choose a color.</p>
                    )}
                </div>

                {/* ── DROPDOWN / OPTION CELL ───────────────────── */}
                <div className="border-t pt-3">
                    <div className="text-xs font-bold text-[#0D6EFD] uppercase tracking-wider mb-2">Dropdown Cell</div>
                    {currentKey ? (
                        <div className="space-y-2">
                            <p className="text-[10px] text-slate-500">Cell: Row {cellPos!.row + 1}, Col {cellPos!.col + 1}</p>
                            <div className="flex gap-1">
                                <button
                                    className={`flex-1 py-1.5 text-[11px] font-medium rounded border transition-colors ${(!cellMetaLocal[currentKey]?.cellType || cellMetaLocal[currentKey]?.cellType === "text") ? "bg-blue-50 border-blue-300 text-[#0D6EFD]" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                                    onClick={() => updateCellMeta(currentKey!, { cellType: "text", dropdownOptions: undefined, selectedValue: undefined })}
                                >📝 Text</button>
                                <button
                                    className={`flex-1 py-1.5 text-[11px] font-medium rounded border transition-colors ${cellMetaLocal[currentKey]?.cellType === "dropdown" ? "bg-blue-50 border-blue-300 text-[#0D6EFD]" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                                    onClick={() => {
                            ...
                            {cellMetaLocal[currentKey]?.cellType === "dropdown" && (
                                <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-2 space-y-2">
                                    {/* Presets */}
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-1 mt-0.5">
                                            {Object.entries(DROPDOWN_PRESETS).map(([name, opts]) => (
                                                <button key={name}
                                                    className="px-2 py-0.5 text-[10px] bg-white border border-slate-200 rounded hover:bg-blue-50 hover:border-blue-300"
                                                    onClick={() => {
                                    ...
                                    <div>
                                        ...
                                        {(cellMetaLocal[currentKey]?.dropdownOptions || []).map((opt: string, i: number) => (
                                            <div key={i} className="flex items-center gap-1 mt-0.5">
                                                <span className="flex-1 text-xs bg-white border border-slate-100 rounded px-2 py-0.5">{opt}</span>
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        />
                                        <button className="px-2 py-1 text-xs bg-[#0D6EFD] text-white rounded hover:bg-[#0D6EFD]"
                                            onClick={() => {
                                    ...
                                    {(cellMetaLocal[currentKey]?.dropdownOptions?.length > 0) && (
                                        <div>
                                            <span className="text-[10px] text-slate-400">Selected value:</span>
                                            <div className="flex flex-wrap gap-1 mt-0.5">
                                                {cellMetaLocal[currentKey]?.dropdownOptions.map((opt: string, i: number) => (
                                                    <button key={i}
                                                        className={`px-2 py-0.5 text-[10px] rounded border transition-colors ${cellMetaLocal[currentKey]?.selectedValue === opt
                                                            ? "bg-[#0D6EFD] text-white border-blue-500"
                                                            : "bg-white text-slate-600 border-slate-200 hover:border-blue-300"}`}
                                                        onClick={() => updateCellMeta(currentKey!, { selectedValue: opt })}
                                                    >{opt}</button>
                                                ))}
                                                <button className="px-1.5 py-0.5 text-[10px] text-slate-400 hover:text-red-500" title="Clear"
                        ...
                        <p className="text-[10px] text-slate-400">Click a cell in edit mode to configure dropdown options.</p>
                    )}
                </div>
            </div>
        );
    ...
    return (
        <div className="flex flex-col space-y-6">
            ...
            <div className="flex flex-col space-y-1">
                ...
                <span className="text-[10px] text-slate-400 text-right">{props.strokeWidth || 1}px</span>
            </div>
        </div>
    );
};
    ...
    return (
        <button
            className={`flex items-center justify-center gap-1.5 px-2 py-1.5 text-[11px] font-medium rounded-md border shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-all focus:outline-none focus:ring-2 focus:ring-offset-1 ${danger
                ...
                : "border-slate-200 text-slate-700 bg-white hover:bg-slate-50 hover:border-blue-300 hover:text-[#0D6EFD] focus:ring-blue-100"
            }`}
            onClick={onClick}
        >
