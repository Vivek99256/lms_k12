...
export const A4PageBlock = ({ children }: any) => {
    ...
    const toolbarButtonClass = "h-8 w-8 inline-flex items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-900 disabled:text-slate-300 disabled:hover:bg-transparent";
    ...
    return (
        ...
        >
            ...
            >
                <button
                    type="button"
                    className="max-w-[280px] truncate text-left text-lg font-semibold text-slate-950"
                    onClick={selectPage}
                    title={`Page ${pageNumber}`}
                >
                    Page {pageNumber} <span className="font-medium text-slate-500">- Add p...</span>
                </button>
            ...
            >
                ...
                {!pageHidden && <div className="absolute inset-8 border border-slate-100/50 pointer-events-none z-0" data-html2canvas-ignore="true" />}
                {pageHidden ? (
                    <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-50 text-sm font-medium text-slate-400" data-html2canvas-ignore="true">
                        Page hidden
                    </div>
                ) : (
