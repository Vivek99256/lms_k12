...
export const LayersPanel = () => {
    ...
    const renderNode = (node: any, depth: number = 0, siblingIndex: number = 0) => {
        ...
        return (
            <div key={node.id}>
                <div
                    className="select-none flex items-center py-1.5 px-2 cursor-pointer transition-colors text-xs hover:bg-slate-100"
                    style={{ paddingLeft: `${depth * 12 + 8}px` }}
                    onClick={() => (actions as any).selectNode(node.id)}
                >
                    <button
                        className={`p-0.5 hover:bg-slate-200 rounded mr-1 transition-colors ${!hasChildren && 'invisible'}`}
                        onClick={(e) => {
                            e.stopPropagation();
                    ...
                    {isEditing ? (
                        <input
                            ...
                            autoFocus
                            className="flex-1 px-1 py-0.5 text-xs border border-blue-500 rounded outline-none"
                        />
                    ) : (
                        <span 
                            className="truncate flex-1 font-medium text-slate-700 cursor-text"
                            onDoubleClick={(e) => {
                                e.stopPropagation();
    ...
    return (
        <div className="w-full">
            {/* Mode Toggle */}
            <div className="px-2 py-2 flex items-center justify-between bg-slate-50 border-b">
            ...
            {viewMode === "layers" ? (
                ...
                <div className="flex flex-col h-full">
                    {/* JSON Action Bar */}
                    <div className="px-2 py-2 flex items-center gap-2 border-b bg-slate-50/50">
                        <ToolButton
                    ...
                    <div className="flex-1 overflow-auto p-2">
                        {isEditing ? (
                            <textarea
                                ...
                                className="w-full h-full min-h-[200px] text-[9px] font-mono text-slate-600 bg-slate-50 border border-slate-200 rounded p-2 resize-none focus:outline-none focus:border-blue-500"
                                spellCheck={false}
                            />
                        ) : (
