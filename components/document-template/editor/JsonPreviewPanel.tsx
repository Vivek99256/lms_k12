...
export const JsonPreviewPanel = ({ onClose }: { onClose?: () => void }) => {
    ...
    const renderTree = (nodes: any[], indent: number = 0, isLast: boolean = true) => {
        return nodes.map((node, idx) => {
            ...
            return (
                <div key={idx}>
                    <div className="text-[10px] font-mono text-slate-700 py-0.5">
                        <span className="text-slate-400">{connector}</span>
                        <span className="font-semibold text-[#0D6EFD]">{node.name}</span>
                        {node.props.width && <span className="text-slate-400"> w:{node.props.width}</span>}
                        {node.props.height && <span className="text-slate-400"> h:{node.props.height}</span>}
                        {node.props.padding && <span className="text-slate-400"> p:{node.props.padding}</span>}
                    </div>
    ...
    return (
        <div className="w-full h-full bg-white flex flex-col relative z-20 shrink-0">
            ...
            {/* Action Bar */}
            <div className="px-3 py-2 flex items-center gap-2 border-b bg-slate-50/50">
                <div className="flex items-center bg-slate-100 rounded-md p-0.5">
                    <button
                        onClick={() => setViewMode("tree")}
                        className={`px-2 py-1 text-[10px] rounded ${viewMode === "tree" ? "bg-white shadow-sm text-[#0D6EFD] font-medium" : "text-slate-500"}`}
                    >
                        Tree
                    </button>
                    <button
                        onClick={() => setViewMode("json")}
                        className={`px-2 py-1 text-[10px] rounded ${viewMode === "json" ? "bg-white shadow-sm text-[#0D6EFD] font-medium" : "text-slate-500"}`}
                    >
                        JSON
                    </button>
                </div>
            ...
            <div className="flex-1 overflow-auto p-3">
                {viewMode === "tree" ? (
                    <div className="font-mono">
                        <div className="text-[10px] font-bold text-slate-800 mb-2">ROOT</div>
                        {jsonOutput && jsonOutput.trim() !== '' && jsonOutput.trim() !== '{}' ? (
                            renderTree(buildTree(jsonOutput))
                        ) : (
                            <div className="text-[10px] text-slate-400 italic">No content in editor</div>
                        )}
                    </div>
                ) : isEditing ? (
                    <textarea
                        ...
                        className="w-full h-full min-h-[400px] text-[10px] font-mono text-slate-600 bg-slate-50 border border-slate-200 rounded p-2 resize-none focus:outline-none focus:border-blue-500"
                        spellCheck={false}
                    />
                ) : (
