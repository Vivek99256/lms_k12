        ...
        <div className="absolute left-6 top-1/2 -translate-y-1/2 z-50 flex">
            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-[28px] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-white max-w-min ring-1 ring-black/[0.03] p-2 flex flex-col gap-2 relative z-10">
                ...
                <button
                    ...
                    className={`p-3 rounded-2xl transition-all duration-200 group ${activeTool === 'select' ? 'bg-blue-50 shadow-inner border border-blue-100/50' : 'hover:bg-blue-50/50 border border-transparent'}`}
                    title="Select (V)"
                >
                    <MousePointer2 className={`w-5 h-5 transition-transform duration-200 ${activeTool === 'select' ? 'text-[#0D6EFD] drop-shadow-md scale-110' : 'text-slate-500 group-hover:text-[#0D6EFD] group-hover:scale-110'}`} />
                </button>
                    ...
                    {hoveredTool === 'draw' && (
                        <div className="absolute left-full top-0 pl-2 h-full z-50">
                            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] border border-white/50 ring-1 ring-black/[0.03] p-2 flex flex-col gap-1 min-w-[130px] animate-in slide-in-from-left-2 fade-in duration-200">
                                <button type="button" onClick={() => setActiveTool('draw_pen')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeTool === 'draw_pen' ? 'bg-blue-50 text-[#0D6EFD]' : 'text-slate-700'}`}>
                                    <Pen className="w-4 h-4 text-slate-800" /> <span className="text-xs font-semibold">Pen</span>
                                </button>
                                <button type="button" onClick={() => setActiveTool('draw_marker')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeTool === 'draw_marker' ? 'bg-blue-50 text-[#0D6EFD]' : 'text-slate-700'}`}>
                                    <PenLine className="w-4 h-4 text-red-500" /> <span className="text-xs font-semibold">Marker</span>
                                </button>
                                <button type="button" onClick={() => setActiveTool('draw_highlighter')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeTool === 'draw_highlighter' ? 'bg-blue-50 text-[#0D6EFD]' : 'text-slate-700'}`}>
                                    <Highlighter className="w-4 h-4 text-yellow-500" /> <span className="text-xs font-semibold">Highlight</span>
                                </button>
                                <div className="h-px bg-slate-100 w-full my-1 border-0"></div>
                                <button type="button" onClick={() => setActiveTool('draw_eraser')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-red-50 hover:text-red-600 transition-colors ${activeTool === 'draw_eraser' ? 'bg-red-50 text-red-600' : 'text-slate-700'}`}>
                ...
                >
                    ...
                    {hoveredTool === 'line' && (
                        <div className="absolute left-full top-0 pl-2 h-full z-50">
                            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] border border-white/50 ring-1 ring-black/[0.03] p-2 flex flex-col gap-1 min-w-[130px] animate-in slide-in-from-left-2 fade-in duration-200">
                                <button type="button" onClick={() => setActiveTool('line_straight')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeTool === 'line_straight' ? 'bg-blue-50 text-[#0D6EFD]' : 'text-slate-700'}`}>
                                    <StraightLineIcon className="w-4 h-4 text-slate-800" /> <span className="text-xs font-semibold">Straight</span>
                                </button>
                                <button type="button" onClick={() => setActiveTool('line_curved')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeTool === 'line_curved' ? 'bg-blue-50 text-[#0D6EFD]' : 'text-slate-700'}`}>
                                    <CurvedLineIcon className="w-4 h-4 text-slate-800" /> <span className="text-xs font-semibold">Curved</span>
                                </button>
                                <button type="button" onClick={() => setActiveTool('line_elbow')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeTool === 'line_elbow' ? 'bg-blue-50 text-[#0D6EFD]' : 'text-slate-700'}`}>
