...
export const EditorCanvas = ({ children, activeTool }: { children: React.ReactNode, activeTool?: WhiteboardTool }) => {
    ...
    return (
        <div
            ref={containerRef}
            className="w-full h-full flex-1 overflow-auto bg-slate-100 flex relative"
            onPointerMove={(e) => {
                (window as any).__craft_mouse_x = e.clientX;
                (window as any).__craft_mouse_y = e.clientY;
            }}
        >
            <div className="absolute bottom-6 right-8 z-50 flex items-center bg-white shadow-lg rounded-full px-2 py-1 space-x-1 border border-slate-200">
                 <button onClick={() => { setZoomMode("manual"); setZoom(z => Math.max(z - 0.1, 0.25)); }} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-600"><ZoomOut className="w-4 h-4" /></button>
                 <span className="text-xs font-semibold w-12 text-center text-slate-700">{Math.round(zoom * 100)}%</span>
                 <button onClick={() => { setZoomMode("manual"); setZoom(z => Math.min(z + 0.1, 2.5)); }} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-600"><ZoomIn className="w-4 h-4" /></button>
                 <div className="w-px h-4 bg-slate-300 mx-1"></div>
                 <button onClick={fitToScreen} className={`px-3 py-1.5 text-xs font-semibold rounded-full transition-colors ${zoomMode === 'fit' ? 'bg-blue-100 text-[#0D6EFD]' : 'hover:bg-slate-100 text-slate-600'}`}>Fit</button>
            </div>
